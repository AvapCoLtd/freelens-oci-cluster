...
# GitHub ミラーと公開リリース。
#
#   ci/github.sh mirror            master とタグを AvapCoLtd/freelens-oci-cluster へ push する
#   ci/github.sh release <タグ>    GitHub Release を作り、*.tgz / *.tgz.sha256 を添付する
#
# 認証は GitHub App「freelens-plugins-release」(Contents RW / このリポジトリのみ)。CI 変数で受け取る:
#   GH_APP_ID           App ID
#   GH_APP_PRIVATE_KEY  秘密鍵(file 型変数。値は bao に保管)
#
# App の installation token は1時間で失効するので、ジョブのたびに発行する。
# 依存は openssl / curl / git だけ(jq は使わない。JSONは素朴に抜く)。
set -eu

REPO="${GH_REPO:-AvapCoLtd/freelens-oci-cluster}"
...
release)
  ...
  body="freelens-oci-cluster ${tag#v} — FreeLens の Extensions 画面にドラッグ&ドロップしてインストール。詳細は README を参照。"
